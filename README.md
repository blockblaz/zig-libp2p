...
| Typed error sets (layers) | Partial | [#45](https://github.com/ch4r10t33r/zig-libp2p/issues/45) — codec layers + **TCP** (`listen` / `dial` / `accept`) and **stream multistream** handshakes return / map to `TransportError` via `transport.transport_error`; zquic lifecycle still embedder-owned |
...
| Module | Role |
|--------|------|
| `errors` | Layered errors: `ReqRespError`, `GossipsubError`, `TransportError` (#45) |
...
| `transport.transport_error` | Maps `std.Io.net` connect/listen/accept, multistream handshake I/O, and `security.libp2p_tls` parse failures into `TransportError` |

...
| Kademlia DHT (kad-dht) | Done | [#93](https://github.com/ch4r10t33r/zig-libp2p/issues/93) — [`kad_dht`](../src/kad_dht/); routing table, iterative lookups, provider store; [docs/KAD_DHT.md](KAD_DHT.md) |
...
- **Releases:** cut **manually** (release-please was removed). Pick the commit on `main`, then `gh release create vX.Y.Z --target main --title vX.Y.Z --notes "…"`. Bump `.version` in [`build.zig.zon`](../build.zig.zon) and the README install snippet in the same change. Tags are the source of truth; consumers (zeam) pin a tag's tarball.

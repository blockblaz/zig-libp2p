...
pub const Stream = struct {
    ...
    pub fn write(self: *Stream, buf: []const u8) WriteError!usize {
        const space = self.output_buf.len - self.out_end;
        const n = @min(buf.len, space);
        if (n == 0 and buf.len > 0) return error.NoSpaceLeft;
        @memcpy(self.output_buf[self.out_end..][0..n], buf[0..n]);
        self.out_end += n;
        return n;
    }

    pub fn writeAll(self: *Stream, buffer: []const u8) WriteError!void {
        var n: usize = 0;
        while (n < buffer.len) {
    ...
    pub fn read(self: *Stream, buffer: []u8) ReadError!usize {
        const avail = self.input_buf.len - self.in_pos;
        const n = @min(buffer.len, avail);
        @memcpy(buffer[0..n], self.input_buf[self.in_pos..][0..n]);
        self.in_pos += n;
        return n;
    }
};

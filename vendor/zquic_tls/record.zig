...
pub const Writer = struct {
    ...
    /// RFC 7301 ALPN (single protocol name).
    pub fn applicationLayerProtocolNegotiation(w: *Writer, protocol: []const u8) !void {
        if (protocol.len > 255) return error.TlsIllegalParameter;
        const list_len: u16 = 1 + @as(u16, @intCast(protocol.len));
        try w.enumValue(proto.Extension.application_layer_protocol_negotiation);
        try w.int(u16, list_len + 2);
        try w.int(u16, list_len);
        try w.int(u8, protocol.len);
        try w.inner.writeAll(protocol);
    }

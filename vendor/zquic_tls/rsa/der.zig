...
pub const Element = struct {
    ...
    pub fn init(bytes: []const u8, index: Index) Error!Element {
        ...
        const identifier = @as(Identifier, @bitCast((reader.take(1) catch return error.EndOfStream)[0]));
        const size_or_len_size = (reader.take(1) catch return error.EndOfStream)[0];
        ...
        var len: Index = 0;
        for (0..len_size) |_| {
            const b = (reader.take(1) catch return error.EndOfStream)[0];
            len = (len << 8) | b;
        }

...
pub const Options = struct {
    ...
    /// Optional ALPN protocol name (RFC 7301), e.g. `"libp2p"` for libp2p TCP TLS.
    alpn: ?[]const u8 = null,
        ...
        pub const Ticket = struct {
            ...
            pub fn init(payload: []const u8, secret: []const u8) !Ticket {
                ...
                return .{
                    ...
                    .payload = payload,
                    .secret = secret,
                    .created_at = @import("tls_compat.zig").nowSec() * 1000,
                };
            }
            ...
            pub fn obfuscatedAge(t: Ticket) u32 {
                const age: u32 = @intCast(@import("tls_compat.zig").nowSec() * 1000 - t.created_at);
...
pub const Handshake = struct {
    ...
    fn initKeys(h: *Self, opt: Options) !void {
        ...
        @import("tls_compat.zig").random.bytes(&buf);
    ...
    fn makeClientHello(h: *Self, opt: Options, resumption_ticket: ?ResumptionTicket) !void {
        ...
        if (opt.alpn) |alpn| try w.applicationLayerProtocolNegotiation(alpn);
...
test "parse tls 1.2 server hello" {
    ...
    h.cert = .{ .host = "example.ulfheim.net", .skip_verify = true, .root_ca = cert.Bundle.empty };
    try h.readServerFlight1();
...
test "verify google.com certificate" {
    ...
    var ca_bundle: Certificate.Bundle = cert.Bundle.empty;
    try ca_bundle.rescan(testing.allocator);
    defer ca_bundle.deinit(testing.allocator);

    h.cert = .{ .host = "google.com", .skip_verify = true, .root_ca = cert.Bundle.empty, .now_sec = 1714846451 };
    try h.readServerFlight1();
    try h.verifyCertificateSignatureTls12();
}
...
pub const NonBlock = struct {
    ...
    /// Peer leaf certificate DER after a successful client handshake (empty before).
    pub fn peerLeafCertificateDer(self: Self) []const u8 {
        return self.inner.cert.leaf_cert_der;
    }
};

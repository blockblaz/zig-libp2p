...
pub const Options = struct {
    ...
    /// Optional ALPN to advertise in EncryptedExtensions (e.g. `"libp2p"`).
    alpn: ?[]const u8 = null,
...
pub const Handshake = struct {
    ...
    fn initKeys(h: *Self, opt: Options) void {
        @import("tls_compat.zig").random.bytes(&h.server_random);
        if (opt.auth) |a| {
    ...
    fn serverFlight(h: *Self, opt: Options) !void {
        ...
        try w.record(.change_cipher_spec, &[_]u8{1});
        {
            ...
            try common.writeEncryptedExtensions(&hw, opt.alpn);
    ...
    inline fn sharedKey(h: *Self) ![]const u8 {
        ...
        @import("tls_compat.zig").random.bytes(&seed);

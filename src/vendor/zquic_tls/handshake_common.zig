...
pub const CertificateParser = struct {
    ...
    /// First certificate DER from the peer's Certificate handshake message (TLS 1.3 leaf).
    leaf_cert_der_buf: [8192]u8 = undefined,
    ...
    pub fn parseCertificate(h: *CertificateParser, d: *record.Decoder, tls_version: proto.Version) !void {
        if (h.now_sec == 0) {
            h.now_sec = @import("tls_compat.zig").nowSec();
        }
        ...
        while (d.idx - start_idx < certs_len) {
            ...
            if (last_cert) |pc| {
                ...
                h.leaf_cert_der = dupe(&h.leaf_cert_der_buf, crt);
                last_cert = subject;
            }
            if (!h.skip_verify) {

    ...
    pub fn expectEnum(self: *Parser, comptime Enum: type) Error!Enum {
        ...
        return Enum.oids.get(oid) orelse {
            if (builtin.mode == .Debug) {
                log.warn("unknown oid for enum {s}\n", .{@typeName(Enum)});
            }
...
pub const Element = struct {
    ...
    pub fn init(bytes: []const u8, index: Index) Error!Element {
        if (index + 2 > bytes.len) return error.InvalidLength;
        const identifier = @as(Identifier, @bitCast(bytes[index]));
        const size_or_len_size = bytes[index + 1];
        ...
        const len_size: u7 = @truncate(size_or_len_size & 0x7f);
        if (len_size == 0 or len_size > 4) return error.InvalidLength;
        if (start + len_size > bytes.len) return error.InvalidLength;
        var len: Index = 0;
        for (bytes[start .. start + len_size]) |b| len = (len << 8) | b;
        start += len_size;
